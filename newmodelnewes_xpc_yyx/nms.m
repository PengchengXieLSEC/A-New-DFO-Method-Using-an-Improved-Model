...
% ----------------------------------------------------------
function [x, fmax, nf] = nms(fun, x, stopit, savit, varargin)
...
k = k+1;
    ...
    if fmax > fmax_old
       ...
       if trace
        %%%xpc fprintf('Iter. %2.0f,', k)
        %%%xpc fprintf(['  how = ' how '  ']);
        %%%xpc fprintf('nf = %3.0f,  f = %9.4e  (%2.1f%%)\n', nf, fmax, ...
        %%%xpc         100*(fmax-fmax_old)/(abs(fmax_old)+eps))
       end
    end
    fmax_old = fmax;
    ...
    if fmax >= stopit(3)
       %%%xpc msg = ['Exceeded target...quitting\n'];
       break  % Quit.
    end

    % Stopping Test 2 - too many f-evals?
    if nf >= stopit(2)
       %%%xpc msg = ['Max no. of function evaluations exceeded...quitting\n'];
       break  % Quit.
    end
    ...
    if size_simplex <= tol
       %%%xpc msg = sprintf('Simplex size %9.4e <= %9.4e...quitting\n', ...
       %%%xpc                size_simplex, tol);
       break  % Quit.
    end
